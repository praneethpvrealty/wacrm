...
// Use service role key if available to bypass RLS during ingestion, otherwise fallback to anon
const SUPABASE_KEY = process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY;

if (!SUPABASE_URL || !SUPABASE_KEY) {
  console.error('Error: Supabase environment variables are missing (NEXT_PUBLIC_SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY).');
  process.exit(1);
}
...
async function run() {
  console.log(`Starting real estate projects ingestion of ${SEEDED_PROJECTS.length} seeds...`);
  ...
  console.log('Ingestion succeeded! Sourced projects are now live in rera_projects.');
}
